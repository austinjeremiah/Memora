export default function Steps() {
  return (
    <>
      <section className="framer-q836dk" id="how-it-works" data-framer-name="Steps">
        <div className="framer-5ytnd" data-framer-name="Container">
          <div className="framer-bfka67" data-framer-name="Heading">
            <div className="ssr-variant">
              <div className="framer-i29br6-container">
                <div className="framer-KVhPv framer-gh8mro framer-v-gh8mro" data-framer-name="Variant 1" style={{width: "100%", opacity: "1"}}>
                  <div className="framer-iw76gh-container" style={{opacity: "1"}}>
                    <div className="framer-xjRP0 framer-n3Cte framer-bodobx framer-v-bodobx" data-framer-name="Section Tag" style={{borderRadius: "1px", opacity: "1"}}>
                      <figure className="framer-rpd40f" data-framer-name="Image" style={{opacity: "0.8"}}>
                        <div style={{position: "absolute", borderRadius: "inherit", cornerShape: "inherit", top: "0", right: "0", bottom: "0", left: "0"}} data-framer-background-image-wrapper="true">
                          <img decoding="auto" loading="lazy" width="24" height="24" src="/images/lwna4wcrpy0htxmtn0iteqrjig.svg" alt="" style={{display: "block", width: "100%", height: "100%", borderRadius: "inherit", cornerShape: "inherit", objectPosition: "center", objectFit: "cover"}} />
                        </div>
                      </figure>
                      <div className="framer-q7ns0q" data-framer-name="FEATURES" data-framer-component-type="RichTextContainer" style={{"--extracted-r6o4lv": "var(--token-e77749d5-1f11-472b-b926-5090d7e5b50e, rgb(132, 145, 171))", "--framer-paragraph-spacing": "0px", transform: "none", opacity: "1"}}>
                        <p className="framer-text framer-styles-preset-1xv0u9n" data-styles-preset="VsDceC7bv" style={{"--framer-text-color": "var(--extracted-r6o4lv, var(--token-e77749d5-1f11-472b-b926-5090d7e5b50e, rgb(132, 145, 171)))"}}>
                          STEPS TO USE
                        </p>
                      </div>
                    </div>
                  </div>
                  <div className="framer-1v7615w" data-framer-name="Line" style={{backgroundColor: "var(--token-f4dc11a3-eab6-45ff-bb5d-90cc77e6a1e2, rgba(125, 164, 255, 0.16))", opacity: "1"}} />
                </div>
              </div>
            </div>
            <div className="framer-1uvspec" data-framer-name="Title">
              <div className="framer-cw73wf" data-framer-name="Main">
                <div className="framer-1hmkxk9" data-framer-name="Alpha Range Technology" data-framer-component-type="RichTextContainer" style={{transform: "none"}}>
                  <h2 className="framer-text framer-styles-preset-mnfyzd" data-styles-preset="muXEgmE57">
                    3 Steps to a Verified Handoff
                  </h2>
                </div>
              </div>
              <div className="framer-gm8yds" data-framer-name="Subtext" data-framer-component-type="RichTextContainer" style={{transform: "none"}}>
                <p className="framer-text framer-styles-preset-1xv0u9n" data-styles-preset="VsDceC7bv" style={{"--framer-text-color": "var(--token-e77749d5-1f11-472b-b926-5090d7e5b50e, rgb(176, 190, 217))"}}>
                  From an LLM's proposal to a clinician-approved, onchain-attested state.
                </p>
              </div>
            </div>
          </div>
          <div className="framer-j3ju4u" data-framer-name="Content">
            <div className="framer-zx8cpx hidden-1pkxm8v hidden-zmbq2w" data-framer-name="Numbers">
              <div className="framer-ym0cs7" data-framer-name="Container">
                <div className="framer-12vbwb0" data-framer-name="Beam">
                  <div className="framer-1r0lx73" data-framer-name="Line" style={{willChange: "transform", opacity: "1", transform: "translateX(-1270px)"}} />
                </div>
                <div className="framer-47huoa" data-framer-name="01">
                  <div className="framer-8k1kxu" data-framer-name="01." data-framer-component-type="RichTextContainer" style={{transform: "none"}}>
                    <p className="framer-text framer-styles-preset-1xv0u9n" data-styles-preset="VsDceC7bv" style={{"--framer-text-color": "var(--token-839225cb-b1fc-470d-a0c2-2eb7fcc590b8, rgb(255, 255, 255))"}}>
                      01.
                    </p>
                  </div>
                </div>
                <div className="framer-vadvac" data-framer-name="02">
                  <div className="framer-904g4x" data-framer-name="02." data-framer-component-type="RichTextContainer" style={{transform: "none"}}>
                    <p className="framer-text framer-styles-preset-1xv0u9n" data-styles-preset="VsDceC7bv" style={{"--framer-text-color": "var(--token-839225cb-b1fc-470d-a0c2-2eb7fcc590b8, rgb(255, 255, 255))"}}>
                      02.
                    </p>
                  </div>
                </div>
                <div className="framer-xe8yhi" data-framer-name="03">
                  <div className="framer-53mota" data-framer-name="03." data-framer-component-type="RichTextContainer" style={{transform: "none"}}>
                    <p className="framer-text framer-styles-preset-1xv0u9n" data-styles-preset="VsDceC7bv" style={{"--framer-text-color": "var(--token-839225cb-b1fc-470d-a0c2-2eb7fcc590b8, rgb(255, 255, 255))"}}>
                      03.
                    </p>
                  </div>
                </div>
              </div>
            </div>
            <div className="framer-1juskmm" data-framer-name="Screens">
              <div className="ssr-variant hidden-1pkxm8v hidden-zmbq2w">
                <div className="framer-19an8tm-container" id="step-01" style={{willChange: "transform", opacity: "1", transform: "none"}}>
                  <div className="framer-ZDn9o framer-GeYI4 framer-n3Cte framer-zdjh5r framer-v-zdjh5r" data-border="true" data-framer-name="Desktop 01" style={{"--border-bottom-width": "1px", "--border-color": "var(--token-f4dc11a3-eab6-45ff-bb5d-90cc77e6a1e2, rgba(125, 164, 255, 0.16))", "--border-left-width": "1px", "--border-right-width": "1px", "--border-style": "solid", "--border-top-width": "1px", backgroundColor: "var(--token-f8eb999f-285c-4229-9c3a-c69e8052e994, rgb(6, 7, 10))", width: "100%", borderRadius: "24px", opacity: "1"}}>
                    <div className="framer-1wmr65e" data-framer-name="Content" style={{opacity: "1"}}>
                      <div className="framer-jmip68" data-framer-name="Title" style={{opacity: "1"}}>
                        <div className="framer-ecxnd" data-framer-name="Sign up and select Plan" data-framer-component-type="RichTextContainer" style={{"--framer-paragraph-spacing": "0px", transform: "none", opacity: "1"}}>
                          <h3 className="framer-text framer-styles-preset-1pjo9hs" data-styles-preset="HuTKvbhX7">
                            Retrieve the memory
                          </h3>
                        </div>
                        <div className="framer-g2oltk" data-framer-name="Fin is the top-performing and most capable AI Agent\u2014handling more complex queries" data-framer-component-type="RichTextContainer" style={{"--extracted-r6o4lv": "var(--token-e77749d5-1f11-472b-b926-5090d7e5b50e, rgb(132, 145, 171))", "--framer-paragraph-spacing": "0px", transform: "none", opacity: "1"}}>
                          <p className="framer-text framer-styles-preset-1xv0u9n" data-styles-preset="VsDceC7bv" style={{"--framer-text-color": "var(--extracted-r6o4lv, var(--token-e77749d5-1f11-472b-b926-5090d7e5b50e, rgb(132, 145, 171)))"}}>
                            Situation, role, and task compile into a retrieval plan against Sibyl's HOT, WARM, and COLD layers.
                          </p>
                        </div>
                      </div>
                      <div className="framer-vbh1j9" data-framer-name="Points" style={{opacity: "1"}}>
                        <div className="framer-a75ite-container" style={{opacity: "1"}}>
                          <div className="framer-oNbXA framer-n3Cte framer-jmdv9y framer-v-9wsqfm" data-framer-name="Big" style={{width: "100%", borderRadius: "12px", opacity: "1"}}>
                            <div className="framer-o1thv5" data-framer-name="Content" style={{opacity: "1"}}>
                              <div className="framer-v3ildb" data-framer-name="Healthcare" data-framer-component-type="RichTextContainer" style={{"--extracted-r6o4lv": "var(--token-7ed67f45-7ffc-4523-9689-9d08f6aa2909, rgb(209, 212, 227))", "--framer-paragraph-spacing": "0px", transform: "none", opacity: "1"}}>
                                <p className="framer-text framer-styles-preset-1xv0u9n" data-styles-preset="VsDceC7bv" style={{"--framer-text-color": "var(--extracted-r6o4lv, var(--token-7ed67f45-7ffc-4523-9689-9d08f6aa2909, rgb(209, 212, 227)))"}}>
                                  HOT — current context, always fresh.
                                </p>
                              </div>
                              <div className="framer-1l4bl6k" data-framer-name="Label" style={{backgroundColor: "var(--token-2765094e-0f8b-477f-9e2a-092fb0171343, rgba(255, 255, 255, 0.1))", borderRadius: "10px", boxShadow: "rgba(255, 255, 255, 0.31) 0px 0px 4px 0px inset", opacity: "1"}}>
                                <div data-framer-component-type="SVG" data-framer-name="SVG" className="framer-vrl8sz" aria-hidden="true" style={{imageRendering: "pixelated", flexShrink: "0", fill: "var(--token-74e333f8-fe87-4945-af87-cae5b7e16c10, rgb(0, 0, 0))", color: "var(--token-74e333f8-fe87-4945-af87-cae5b7e16c10, rgb(0, 0, 0))", opacity: "1"}}>
                                  <div className="svgContainer" style={{width: "100%", height: "100%", aspectRatio: "inherit"}}>
                                    <svg style={{width: "100%", height: "100%"}} viewBox="0 0 15 11" preserveAspectRatio="none" width="100%" height="100%">
                                      <use href="#svg830506521_425" />
                                    </svg>
                                  </div>
                                </div>
                              </div>
                            </div>
                            <div className="framer-35r8qv" data-border="true" data-framer-name="Border" style={{"--border-bottom-width": "1px", "--border-color": "var(--token-f4dc11a3-eab6-45ff-bb5d-90cc77e6a1e2, rgba(125, 164, 255, 0.16))", "--border-left-width": "1px", "--border-right-width": "1px", "--border-style": "solid", "--border-top-width": "1px", backgroundColor: "var(--token-365216f8-7ee6-4f9f-94e0-ca7d584e4354, rgba(255, 254, 250, 0))", borderRadius: "12px", opacity: "1"}} />
                          </div>
                        </div>
                        <div className="framer-3iwn2q-container" style={{opacity: "1"}}>
                          <div className="framer-oNbXA framer-n3Cte framer-jmdv9y framer-v-9wsqfm" data-framer-name="Big" style={{width: "100%", borderRadius: "12px", opacity: "1"}}>
                            <div className="framer-o1thv5" data-framer-name="Content" style={{opacity: "1"}}>
                              <div className="framer-v3ildb" data-framer-name="Healthcare" data-framer-component-type="RichTextContainer" style={{"--extracted-r6o4lv": "var(--token-7ed67f45-7ffc-4523-9689-9d08f6aa2909, rgb(209, 212, 227))", "--framer-paragraph-spacing": "0px", transform: "none", opacity: "1"}}>
                                <p className="framer-text framer-styles-preset-1xv0u9n" data-styles-preset="VsDceC7bv" style={{"--framer-text-color": "var(--extracted-r6o4lv, var(--token-7ed67f45-7ffc-4523-9689-9d08f6aa2909, rgb(209, 212, 227)))"}}>
                                  WARM — current facts, one per entity.
                                </p>
                              </div>
                              <div className="framer-1l4bl6k" data-framer-name="Label" style={{backgroundColor: "var(--token-2765094e-0f8b-477f-9e2a-092fb0171343, rgba(255, 255, 255, 0.1))", borderRadius: "10px", boxShadow: "rgba(255, 255, 255, 0.31) 0px 0px 4px 0px inset", opacity: "1"}}>
                                <div data-framer-component-type="SVG" data-framer-name="SVG" className="framer-vrl8sz" aria-hidden="true" style={{imageRendering: "pixelated", flexShrink: "0", fill: "var(--token-74e333f8-fe87-4945-af87-cae5b7e16c10, rgb(0, 0, 0))", color: "var(--token-74e333f8-fe87-4945-af87-cae5b7e16c10, rgb(0, 0, 0))", opacity: "1"}}>
                                  <div className="svgContainer" style={{width: "100%", height: "100%", aspectRatio: "inherit"}}>
                                    <svg style={{width: "100%", height: "100%"}} viewBox="0 0 15 11" preserveAspectRatio="none" width="100%" height="100%">
                                      <use href="#svg830506521_425" />
                                    </svg>
                                  </div>
                                </div>
                              </div>
                            </div>
                            <div className="framer-35r8qv" data-border="true" data-framer-name="Border" style={{"--border-bottom-width": "1px", "--border-color": "var(--token-f4dc11a3-eab6-45ff-bb5d-90cc77e6a1e2, rgba(125, 164, 255, 0.16))", "--border-left-width": "1px", "--border-right-width": "1px", "--border-style": "solid", "--border-top-width": "1px", backgroundColor: "var(--token-365216f8-7ee6-4f9f-94e0-ca7d584e4354, rgba(255, 254, 250, 0))", borderRadius: "12px", opacity: "1"}} />
                          </div>
                        </div>
                        <div className="framer-1slojwe-container" style={{opacity: "1"}}>
                          <div className="framer-oNbXA framer-n3Cte framer-jmdv9y framer-v-9wsqfm" data-framer-name="Big" style={{width: "100%", borderRadius: "12px", opacity: "1"}}>
                            <div className="framer-o1thv5" data-framer-name="Content" style={{opacity: "1"}}>
                              <div className="framer-v3ildb" data-framer-name="Healthcare" data-framer-component-type="RichTextContainer" style={{"--extracted-r6o4lv": "var(--token-7ed67f45-7ffc-4523-9689-9d08f6aa2909, rgb(209, 212, 227))", "--framer-paragraph-spacing": "0px", transform: "none", opacity: "1"}}>
                                <p className="framer-text framer-styles-preset-1xv0u9n" data-styles-preset="VsDceC7bv" style={{"--framer-text-color": "var(--extracted-r6o4lv, var(--token-7ed67f45-7ffc-4523-9689-9d08f6aa2909, rgb(209, 212, 227)))"}}>
                                  COLD — append-only journal, never deleted.
                                </p>
                              </div>
                              <div className="framer-1l4bl6k" data-framer-name="Label" style={{backgroundColor: "var(--token-2765094e-0f8b-477f-9e2a-092fb0171343, rgba(255, 255, 255, 0.1))", borderRadius: "10px", boxShadow: "rgba(255, 255, 255, 0.31) 0px 0px 4px 0px inset", opacity: "1"}}>
                                <div data-framer-component-type="SVG" data-framer-name="SVG" className="framer-vrl8sz" aria-hidden="true" style={{imageRendering: "pixelated", flexShrink: "0", fill: "var(--token-74e333f8-fe87-4945-af87-cae5b7e16c10, rgb(0, 0, 0))", color: "var(--token-74e333f8-fe87-4945-af87-cae5b7e16c10, rgb(0, 0, 0))", opacity: "1"}}>
                                  <div className="svgContainer" style={{width: "100%", height: "100%", aspectRatio: "inherit"}}>
                                    <svg style={{width: "100%", height: "100%"}} viewBox="0 0 15 11" preserveAspectRatio="none" width="100%" height="100%">
                                      <use href="#svg830506521_425" />
                                    </svg>
                                  </div>
                                </div>
                              </div>
                            </div>
                            <div className="framer-35r8qv" data-border="true" data-framer-name="Border" style={{"--border-bottom-width": "1px", "--border-color": "var(--token-f4dc11a3-eab6-45ff-bb5d-90cc77e6a1e2, rgba(125, 164, 255, 0.16))", "--border-left-width": "1px", "--border-right-width": "1px", "--border-style": "solid", "--border-top-width": "1px", backgroundColor: "var(--token-365216f8-7ee6-4f9f-94e0-ca7d584e4354, rgba(255, 254, 250, 0))", borderRadius: "12px", opacity: "1"}} />
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="framer-1955mog" data-framer-name="Image" style={{mask: "linear-gradient(90deg, rgba(0, 0, 0, 0) 0%, rgb(0, 0, 0) 11.2349%, rgb(0, 0, 0) 100%)", opacity: "1"}}>
                      <figure className="framer-1wnk26" data-framer-name="Image" style={{filter: "brightness(1.5)", mask: "none", opacity: "1"}}>
                        <div style={{position: "absolute", borderRadius: "inherit", cornerShape: "inherit", top: "0", right: "0", bottom: "0", left: "0"}} data-framer-background-image-wrapper="true">
                          <img decoding="auto" loading="lazy" width="1256" height="1076" sizes="(min-width: 1200px) 657.5px, (min-width: 810px) and (max-width: 1199.98px) max(min(max(100vw - 80px, 1px), 1200px), 1px), (max-width: 809.98px) max(min(max(100vw - 40px, 1px), 1200px), 1px)" srcSet="/images/vuighc7sr4a4srqmuodr2uxnby.png 512w, /images/vuighc7sr4a4srqmuodr2uxnby.png 1024w, /images/vuighc7sr4a4srqmuodr2uxnby.png 1256w" src="/images/vuighc7sr4a4srqmuodr2uxnby.png" alt="" style={{display: "block", width: "100%", height: "100%", borderRadius: "inherit", cornerShape: "inherit", objectPosition: "center", objectFit: "cover"}} />
                        </div>
                      </figure>
                    </div>
                  </div>
                </div>
              </div>
              <div className="ssr-variant hidden-1pkxm8v hidden-zmbq2w">
                <div className="framer-1x5hqty-container" id="step-02" style={{willChange: "transform", opacity: "1", transform: "none"}}>
                  <div className="framer-ZDn9o framer-GeYI4 framer-n3Cte framer-zdjh5r framer-v-zdjh5r" data-border="true" data-framer-name="Desktop 01" style={{"--border-bottom-width": "1px", "--border-color": "var(--token-f4dc11a3-eab6-45ff-bb5d-90cc77e6a1e2, rgba(125, 164, 255, 0.16))", "--border-left-width": "1px", "--border-right-width": "1px", "--border-style": "solid", "--border-top-width": "1px", backgroundColor: "var(--token-f8eb999f-285c-4229-9c3a-c69e8052e994, rgb(6, 7, 10))", width: "100%", borderRadius: "24px", opacity: "1"}}>
                    <div className="framer-1wmr65e" data-framer-name="Content" style={{opacity: "1"}}>
                      <div className="framer-jmip68" data-framer-name="Title" style={{opacity: "1"}}>
                        <div className="framer-ecxnd" data-framer-name="Sign up and select Plan" data-framer-component-type="RichTextContainer" style={{"--framer-paragraph-spacing": "0px", transform: "none", opacity: "1"}}>
                          <h3 className="framer-text framer-styles-preset-1pjo9hs" data-styles-preset="HuTKvbhX7">
                            Propose and check the claim
                          </h3>
                        </div>
                        <div className="framer-g2oltk" data-framer-name="Fin is the top-performing and most capable AI Agent\u2014handling more complex queries" data-framer-component-type="RichTextContainer" style={{"--extracted-r6o4lv": "var(--token-e77749d5-1f11-472b-b926-5090d7e5b50e, rgb(132, 145, 171))", "--framer-paragraph-spacing": "0px", transform: "none", opacity: "1"}}>
                          <p className="framer-text framer-styles-preset-1xv0u9n" data-styles-preset="VsDceC7bv" style={{"--framer-text-color": "var(--extracted-r6o4lv, var(--token-e77749d5-1f11-472b-b926-5090d7e5b50e, rgb(132, 145, 171)))"}}>
                            The LLM proposes a clinical brief; the Evidence Resolver checks every claim against the real Sibyl record.
                          </p>
                        </div>
                      </div>
                      <div className="framer-vbh1j9" data-framer-name="Points" style={{opacity: "1"}}>
                        <div className="framer-a75ite-container" style={{opacity: "1"}}>
                          <div className="framer-oNbXA framer-n3Cte framer-jmdv9y framer-v-9wsqfm" data-framer-name="Big" style={{width: "100%", borderRadius: "12px", opacity: "1"}}>
                            <div className="framer-o1thv5" data-framer-name="Content" style={{opacity: "1"}}>
                              <div className="framer-v3ildb" data-framer-name="Healthcare" data-framer-component-type="RichTextContainer" style={{"--extracted-r6o4lv": "var(--token-7ed67f45-7ffc-4523-9689-9d08f6aa2909, rgb(209, 212, 227))", "--framer-paragraph-spacing": "0px", transform: "none", opacity: "1"}}>
                                <p className="framer-text framer-styles-preset-1xv0u9n" data-styles-preset="VsDceC7bv" style={{"--framer-text-color": "var(--extracted-r6o4lv, var(--token-7ed67f45-7ffc-4523-9689-9d08f6aa2909, rgb(209, 212, 227)))"}}>
                                  Evidence-linked — source, event ID, timestamp.
                                </p>
                              </div>
                              <div className="framer-1l4bl6k" data-framer-name="Label" style={{backgroundColor: "var(--token-2765094e-0f8b-477f-9e2a-092fb0171343, rgba(255, 255, 255, 0.1))", borderRadius: "10px", boxShadow: "rgba(255, 255, 255, 0.31) 0px 0px 4px 0px inset", opacity: "1"}}>
                                <div data-framer-component-type="SVG" data-framer-name="SVG" className="framer-vrl8sz" aria-hidden="true" style={{imageRendering: "pixelated", flexShrink: "0", fill: "var(--token-74e333f8-fe87-4945-af87-cae5b7e16c10, rgb(0, 0, 0))", color: "var(--token-74e333f8-fe87-4945-af87-cae5b7e16c10, rgb(0, 0, 0))", opacity: "1"}}>
                                  <div className="svgContainer" style={{width: "100%", height: "100%", aspectRatio: "inherit"}}>
                                    <svg style={{width: "100%", height: "100%"}} viewBox="0 0 15 11" preserveAspectRatio="none" width="100%" height="100%">
                                      <use href="#svg830506521_425" />
                                    </svg>
                                  </div>
                                </div>
                              </div>
                            </div>
                            <div className="framer-35r8qv" data-border="true" data-framer-name="Border" style={{"--border-bottom-width": "1px", "--border-color": "var(--token-f4dc11a3-eab6-45ff-bb5d-90cc77e6a1e2, rgba(125, 164, 255, 0.16))", "--border-left-width": "1px", "--border-right-width": "1px", "--border-style": "solid", "--border-top-width": "1px", backgroundColor: "var(--token-365216f8-7ee6-4f9f-94e0-ca7d584e4354, rgba(255, 254, 250, 0))", borderRadius: "12px", opacity: "1"}} />
                          </div>
                        </div>
                        <div className="framer-3iwn2q-container" style={{opacity: "1"}}>
                          <div className="framer-oNbXA framer-n3Cte framer-jmdv9y framer-v-9wsqfm" data-framer-name="Big" style={{width: "100%", borderRadius: "12px", opacity: "1"}}>
                            <div className="framer-o1thv5" data-framer-name="Content" style={{opacity: "1"}}>
                              <div className="framer-v3ildb" data-framer-name="Healthcare" data-framer-component-type="RichTextContainer" style={{"--extracted-r6o4lv": "var(--token-7ed67f45-7ffc-4523-9689-9d08f6aa2909, rgb(209, 212, 227))", "--framer-paragraph-spacing": "0px", transform: "none", opacity: "1"}}>
                                <p className="framer-text framer-styles-preset-1xv0u9n" data-styles-preset="VsDceC7bv" style={{"--framer-text-color": "var(--extracted-r6o4lv, var(--token-7ed67f45-7ffc-4523-9689-9d08f6aa2909, rgb(209, 212, 227)))"}}>
                                  Zero unsupported claims reach a clinician.
                                </p>
                              </div>
                              <div className="framer-1l4bl6k" data-framer-name="Label" style={{backgroundColor: "var(--token-2765094e-0f8b-477f-9e2a-092fb0171343, rgba(255, 255, 255, 0.1))", borderRadius: "10px", boxShadow: "rgba(255, 255, 255, 0.31) 0px 0px 4px 0px inset", opacity: "1"}}>
                                <div data-framer-component-type="SVG" data-framer-name="SVG" className="framer-vrl8sz" aria-hidden="true" style={{imageRendering: "pixelated", flexShrink: "0", fill: "var(--token-74e333f8-fe87-4945-af87-cae5b7e16c10, rgb(0, 0, 0))", color: "var(--token-74e333f8-fe87-4945-af87-cae5b7e16c10, rgb(0, 0, 0))", opacity: "1"}}>
                                  <div className="svgContainer" style={{width: "100%", height: "100%", aspectRatio: "inherit"}}>
                                    <svg style={{width: "100%", height: "100%"}} viewBox="0 0 15 11" preserveAspectRatio="none" width="100%" height="100%">
                                      <use href="#svg830506521_425" />
                                    </svg>
                                  </div>
                                </div>
                              </div>
                            </div>
                            <div className="framer-35r8qv" data-border="true" data-framer-name="Border" style={{"--border-bottom-width": "1px", "--border-color": "var(--token-f4dc11a3-eab6-45ff-bb5d-90cc77e6a1e2, rgba(125, 164, 255, 0.16))", "--border-left-width": "1px", "--border-right-width": "1px", "--border-style": "solid", "--border-top-width": "1px", backgroundColor: "var(--token-365216f8-7ee6-4f9f-94e0-ca7d584e4354, rgba(255, 254, 250, 0))", borderRadius: "12px", opacity: "1"}} />
                          </div>
                        </div>
                        <div className="framer-1slojwe-container" style={{opacity: "1"}}>
                          <div className="framer-oNbXA framer-n3Cte framer-jmdv9y framer-v-9wsqfm" data-framer-name="Big" style={{width: "100%", borderRadius: "12px", opacity: "1"}}>
                            <div className="framer-o1thv5" data-framer-name="Content" style={{opacity: "1"}}>
                              <div className="framer-v3ildb" data-framer-name="Healthcare" data-framer-component-type="RichTextContainer" style={{"--extracted-r6o4lv": "var(--token-7ed67f45-7ffc-4523-9689-9d08f6aa2909, rgb(209, 212, 227))", "--framer-paragraph-spacing": "0px", transform: "none", opacity: "1"}}>
                                <p className="framer-text framer-styles-preset-1xv0u9n" data-styles-preset="VsDceC7bv" style={{"--framer-text-color": "var(--extracted-r6o4lv, var(--token-7ed67f45-7ffc-4523-9689-9d08f6aa2909, rgb(209, 212, 227)))"}}>
                                  Policy and authority checked before ALLOW.
                                </p>
                              </div>
                              <div className="framer-1l4bl6k" data-framer-name="Label" style={{backgroundColor: "var(--token-2765094e-0f8b-477f-9e2a-092fb0171343, rgba(255, 255, 255, 0.1))", borderRadius: "10px", boxShadow: "rgba(255, 255, 255, 0.31) 0px 0px 4px 0px inset", opacity: "1"}}>
                                <div data-framer-component-type="SVG" data-framer-name="SVG" className="framer-vrl8sz" aria-hidden="true" style={{imageRendering: "pixelated", flexShrink: "0", fill: "var(--token-74e333f8-fe87-4945-af87-cae5b7e16c10, rgb(0, 0, 0))", color: "var(--token-74e333f8-fe87-4945-af87-cae5b7e16c10, rgb(0, 0, 0))", opacity: "1"}}>
                                  <div className="svgContainer" style={{width: "100%", height: "100%", aspectRatio: "inherit"}}>
                                    <svg style={{width: "100%", height: "100%"}} viewBox="0 0 15 11" preserveAspectRatio="none" width="100%" height="100%">
                                      <use href="#svg830506521_425" />
                                    </svg>
                                  </div>
                                </div>
                              </div>
                            </div>
                            <div className="framer-35r8qv" data-border="true" data-framer-name="Border" style={{"--border-bottom-width": "1px", "--border-color": "var(--token-f4dc11a3-eab6-45ff-bb5d-90cc77e6a1e2, rgba(125, 164, 255, 0.16))", "--border-left-width": "1px", "--border-right-width": "1px", "--border-style": "solid", "--border-top-width": "1px", backgroundColor: "var(--token-365216f8-7ee6-4f9f-94e0-ca7d584e4354, rgba(255, 254, 250, 0))", borderRadius: "12px", opacity: "1"}} />
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="framer-1955mog" data-framer-name="Image" style={{mask: "linear-gradient(90deg, rgba(0, 0, 0, 0) 0%, rgb(0, 0, 0) 11.2349%, rgb(0, 0, 0) 100%)", opacity: "1"}}>
                      <figure className="framer-1wnk26" data-framer-name="Image" style={{filter: "brightness(1.5)", mask: "none", opacity: "1"}}>
                        <div style={{position: "absolute", borderRadius: "inherit", cornerShape: "inherit", top: "0", right: "0", bottom: "0", left: "0"}} data-framer-background-image-wrapper="true">
                          <img decoding="auto" loading="lazy" width="942" height="807" sizes="(min-width: 1200px) 657.5px, (min-width: 810px) and (max-width: 1199.98px) max(min(max(100vw - 80px, 1px), 1200px), 1px), (max-width: 809.98px) max(min(max(100vw - 40px, 1px), 1200px), 1px)" srcSet="/images/1h2g2ankrhxzzmsq2umwuibssys.png 512w, /images/1h2g2ankrhxzzmsq2umwuibssys.png 942w" src="/images/1h2g2ankrhxzzmsq2umwuibssys.png" alt="" style={{display: "block", width: "100%", height: "100%", borderRadius: "inherit", cornerShape: "inherit", objectPosition: "center", objectFit: "cover"}} />
                        </div>
                      </figure>
                    </div>
                  </div>
                </div>
              </div>
              <div className="ssr-variant hidden-1pkxm8v hidden-zmbq2w">
                <div className="framer-5y0zbm-container" id="step-03">
                  <div className="framer-ZDn9o framer-GeYI4 framer-n3Cte framer-zdjh5r framer-v-zdjh5r" data-border="true" data-framer-name="Desktop 01" style={{"--border-bottom-width": "1px", "--border-color": "var(--token-f4dc11a3-eab6-45ff-bb5d-90cc77e6a1e2, rgba(125, 164, 255, 0.16))", "--border-left-width": "1px", "--border-right-width": "1px", "--border-style": "solid", "--border-top-width": "1px", backgroundColor: "var(--token-f8eb999f-285c-4229-9c3a-c69e8052e994, rgb(6, 7, 10))", width: "100%", borderRadius: "24px", opacity: "1"}}>
                    <div className="framer-1wmr65e" data-framer-name="Content" style={{opacity: "1"}}>
                      <div className="framer-jmip68" data-framer-name="Title" style={{opacity: "1"}}>
                        <div className="framer-ecxnd" data-framer-name="Sign up and select Plan" data-framer-component-type="RichTextContainer" style={{"--framer-paragraph-spacing": "0px", transform: "none", opacity: "1"}}>
                          <h3 className="framer-text framer-styles-preset-1pjo9hs" data-styles-preset="HuTKvbhX7">
                            Approve and attest
                          </h3>
                        </div>
                        <div className="framer-g2oltk" data-framer-name="Fin is the top-performing and most capable AI Agent\u2014handling more complex queries" data-framer-component-type="RichTextContainer" style={{"--extracted-r6o4lv": "var(--token-e77749d5-1f11-472b-b926-5090d7e5b50e, rgb(132, 145, 171))", "--framer-paragraph-spacing": "0px", transform: "none", opacity: "1"}}>
                          <p className="framer-text framer-styles-preset-1xv0u9n" data-styles-preset="VsDceC7bv" style={{"--framer-text-color": "var(--extracted-r6o4lv, var(--token-e77749d5-1f11-472b-b926-5090d7e5b50e, rgb(132, 145, 171)))"}}>
                            A clinician reviews and approves; the state is signed EIP-712 and recorded on Base Sepolia.
                          </p>
                        </div>
                      </div>
                      <div className="framer-vbh1j9" data-framer-name="Points" style={{opacity: "1"}}>
                        <div className="framer-a75ite-container" style={{opacity: "1"}}>
                          <div className="framer-oNbXA framer-n3Cte framer-jmdv9y framer-v-9wsqfm" data-framer-name="Big" style={{width: "100%", borderRadius: "12px", opacity: "1"}}>
                            <div className="framer-o1thv5" data-framer-name="Content" style={{opacity: "1"}}>
                              <div className="framer-v3ildb" data-framer-name="Healthcare" data-framer-component-type="RichTextContainer" style={{"--extracted-r6o4lv": "var(--token-7ed67f45-7ffc-4523-9689-9d08f6aa2909, rgb(209, 212, 227))", "--framer-paragraph-spacing": "0px", transform: "none", opacity: "1"}}>
                                <p className="framer-text framer-styles-preset-1xv0u9n" data-styles-preset="VsDceC7bv" style={{"--framer-text-color": "var(--extracted-r6o4lv, var(--token-7ed67f45-7ffc-4523-9689-9d08f6aa2909, rgb(209, 212, 227)))"}}>
                                  Nonce and deadline block any replay.
                                </p>
                              </div>
                              <div className="framer-1l4bl6k" data-framer-name="Label" style={{backgroundColor: "var(--token-2765094e-0f8b-477f-9e2a-092fb0171343, rgba(255, 255, 255, 0.1))", borderRadius: "10px", boxShadow: "rgba(255, 255, 255, 0.31) 0px 0px 4px 0px inset", opacity: "1"}}>
                                <div data-framer-component-type="SVG" data-framer-name="SVG" className="framer-vrl8sz" aria-hidden="true" style={{imageRendering: "pixelated", flexShrink: "0", fill: "var(--token-74e333f8-fe87-4945-af87-cae5b7e16c10, rgb(0, 0, 0))", color: "var(--token-74e333f8-fe87-4945-af87-cae5b7e16c10, rgb(0, 0, 0))", opacity: "1"}}>
                                  <div className="svgContainer" style={{width: "100%", height: "100%", aspectRatio: "inherit"}}>
                                    <svg style={{width: "100%", height: "100%"}} viewBox="0 0 15 11" preserveAspectRatio="none" width="100%" height="100%">
                                      <use href="#svg830506521_425" />
                                    </svg>
                                  </div>
                                </div>
                              </div>
                            </div>
                            <div className="framer-35r8qv" data-border="true" data-framer-name="Border" style={{"--border-bottom-width": "1px", "--border-color": "var(--token-f4dc11a3-eab6-45ff-bb5d-90cc77e6a1e2, rgba(125, 164, 255, 0.16))", "--border-left-width": "1px", "--border-right-width": "1px", "--border-style": "solid", "--border-top-width": "1px", backgroundColor: "var(--token-365216f8-7ee6-4f9f-94e0-ca7d584e4354, rgba(255, 254, 250, 0))", borderRadius: "12px", opacity: "1"}} />
                          </div>
                        </div>
                        <div className="framer-3iwn2q-container" style={{opacity: "1"}}>
                          <div className="framer-oNbXA framer-n3Cte framer-jmdv9y framer-v-9wsqfm" data-framer-name="Big" style={{width: "100%", borderRadius: "12px", opacity: "1"}}>
                            <div className="framer-o1thv5" data-framer-name="Content" style={{opacity: "1"}}>
                              <div className="framer-v3ildb" data-framer-name="Healthcare" data-framer-component-type="RichTextContainer" style={{"--extracted-r6o4lv": "var(--token-7ed67f45-7ffc-4523-9689-9d08f6aa2909, rgb(209, 212, 227))", "--framer-paragraph-spacing": "0px", transform: "none", opacity: "1"}}>
                                <p className="framer-text framer-styles-preset-1xv0u9n" data-styles-preset="VsDceC7bv" style={{"--framer-text-color": "var(--extracted-r6o4lv, var(--token-7ed67f45-7ffc-4523-9689-9d08f6aa2909, rgb(209, 212, 227)))"}}>
                                  Tamper-evident and publicly verifiable.
                                </p>
                              </div>
                              <div className="framer-1l4bl6k" data-framer-name="Label" style={{backgroundColor: "var(--token-2765094e-0f8b-477f-9e2a-092fb0171343, rgba(255, 255, 255, 0.1))", borderRadius: "10px", boxShadow: "rgba(255, 255, 255, 0.31) 0px 0px 4px 0px inset", opacity: "1"}}>
                                <div data-framer-component-type="SVG" data-framer-name="SVG" className="framer-vrl8sz" aria-hidden="true" style={{imageRendering: "pixelated", flexShrink: "0", fill: "var(--token-74e333f8-fe87-4945-af87-cae5b7e16c10, rgb(0, 0, 0))", color: "var(--token-74e333f8-fe87-4945-af87-cae5b7e16c10, rgb(0, 0, 0))", opacity: "1"}}>
                                  <div className="svgContainer" style={{width: "100%", height: "100%", aspectRatio: "inherit"}}>
                                    <svg style={{width: "100%", height: "100%"}} viewBox="0 0 15 11" preserveAspectRatio="none" width="100%" height="100%">
                                      <use href="#svg830506521_425" />
                                    </svg>
                                  </div>
                                </div>
                              </div>
                            </div>
                            <div className="framer-35r8qv" data-border="true" data-framer-name="Border" style={{"--border-bottom-width": "1px", "--border-color": "var(--token-f4dc11a3-eab6-45ff-bb5d-90cc77e6a1e2, rgba(125, 164, 255, 0.16))", "--border-left-width": "1px", "--border-right-width": "1px", "--border-style": "solid", "--border-top-width": "1px", backgroundColor: "var(--token-365216f8-7ee6-4f9f-94e0-ca7d584e4354, rgba(255, 254, 250, 0))", borderRadius: "12px", opacity: "1"}} />
                          </div>
                        </div>
                        <div className="framer-1slojwe-container" style={{opacity: "1"}}>
                          <div className="framer-oNbXA framer-n3Cte framer-jmdv9y framer-v-9wsqfm" data-framer-name="Big" style={{width: "100%", borderRadius: "12px", opacity: "1"}}>
                            <div className="framer-o1thv5" data-framer-name="Content" style={{opacity: "1"}}>
                              <div className="framer-v3ildb" data-framer-name="Healthcare" data-framer-component-type="RichTextContainer" style={{"--extracted-r6o4lv": "var(--token-7ed67f45-7ffc-4523-9689-9d08f6aa2909, rgb(209, 212, 227))", "--framer-paragraph-spacing": "0px", transform: "none", opacity: "1"}}>
                                <p className="framer-text framer-styles-preset-1xv0u9n" data-styles-preset="VsDceC7bv" style={{"--framer-text-color": "var(--extracted-r6o4lv, var(--token-7ed67f45-7ffc-4523-9689-9d08f6aa2909, rgb(209, 212, 227)))"}}>
                                  Sentinel findings use the same flow.
                                </p>
                              </div>
                              <div className="framer-1l4bl6k" data-framer-name="Label" style={{backgroundColor: "var(--token-2765094e-0f8b-477f-9e2a-092fb0171343, rgba(255, 255, 255, 0.1))", borderRadius: "10px", boxShadow: "rgba(255, 255, 255, 0.31) 0px 0px 4px 0px inset", opacity: "1"}}>
                                <div data-framer-component-type="SVG" data-framer-name="SVG" className="framer-vrl8sz" aria-hidden="true" style={{imageRendering: "pixelated", flexShrink: "0", fill: "var(--token-74e333f8-fe87-4945-af87-cae5b7e16c10, rgb(0, 0, 0))", color: "var(--token-74e333f8-fe87-4945-af87-cae5b7e16c10, rgb(0, 0, 0))", opacity: "1"}}>
                                  <div className="svgContainer" style={{width: "100%", height: "100%", aspectRatio: "inherit"}}>
                                    <svg style={{width: "100%", height: "100%"}} viewBox="0 0 15 11" preserveAspectRatio="none" width="100%" height="100%">
                                      <use href="#svg830506521_425" />
                                    </svg>
                                  </div>
                                </div>
                              </div>
                            </div>
                            <div className="framer-35r8qv" data-border="true" data-framer-name="Border" style={{"--border-bottom-width": "1px", "--border-color": "var(--token-f4dc11a3-eab6-45ff-bb5d-90cc77e6a1e2, rgba(125, 164, 255, 0.16))", "--border-left-width": "1px", "--border-right-width": "1px", "--border-style": "solid", "--border-top-width": "1px", backgroundColor: "var(--token-365216f8-7ee6-4f9f-94e0-ca7d584e4354, rgba(255, 254, 250, 0))", borderRadius: "12px", opacity: "1"}} />
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="framer-1955mog" data-framer-name="Image" style={{mask: "linear-gradient(90deg, rgba(0, 0, 0, 0) 0%, rgb(0, 0, 0) 11.2349%, rgb(0, 0, 0) 100%)", opacity: "1"}}>
                      <figure className="framer-1wnk26" data-framer-name="Image" style={{filter: "brightness(1.5)", mask: "none", opacity: "1"}}>
                        <div style={{position: "absolute", borderRadius: "inherit", cornerShape: "inherit", top: "0", right: "0", bottom: "0", left: "0"}} data-framer-background-image-wrapper="true">
                          <img decoding="auto" loading="lazy" width="1256" height="1076" sizes="(min-width: 1200px) 657.5px, (min-width: 810px) and (max-width: 1199.98px) max(min(max(100vw - 80px, 1px), 1200px), 1px), (max-width: 809.98px) max(min(max(100vw - 40px, 1px), 1200px), 1px)" srcSet="/images/8kzdlcsnott5lxj1uywasrczu3e.png 512w, /images/8kzdlcsnott5lxj1uywasrczu3e.png 1024w, /images/8kzdlcsnott5lxj1uywasrczu3e.png 1256w" src="/images/8kzdlcsnott5lxj1uywasrczu3e.png" alt="" style={{display: "block", width: "100%", height: "100%", borderRadius: "inherit", cornerShape: "inherit", objectPosition: "center", objectFit: "cover"}} />
                        </div>
                      </figure>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>
    </>
  );
}
